...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "md:grid-row-7 mx-auto grid grid-cols-1 gap-4 md:mx-[100px] md:grid-cols-6 lg:grid-cols-5 lg:gap-9",
        className,
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("nnoupingnouping@gmail.com");
    setCopied(true);
        ...
        >
          ...
          {id === 3 && (
            <div className="absolute -right-3 flex w-fit gap-1 lg:-right-2 lg:gap-5">
              ...
              <div className="flex flex-col gap-3 lg:gap-8">
                <span className="rounded-lg bg-[#10132e] px-3 py-4 text-center" />
                {["React Native", "MongoDb", "TailwindCSS"].map((item) => (
                  <span
                    className="rounded-lg bg-[#10132E] px-3 py-2 text-center text-xs opacity-50 lg:px-3 lg:py-4 lg:text-base lg:opacity-100"
                    key={item}
                  >
